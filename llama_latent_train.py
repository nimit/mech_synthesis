    ...
    def forward(self, logits_flat, targets_flat):
        ...
        if is_bin.any():
            ...
            sel_logits = logits_flat[idx_bin][
                :, self.bin_start_id : self.bin_end_id + 1
            ]  # (Nb, num_bins)
...
def debug_batch(
    ...
    is_bin = (targets_flat >= BIN_START) & (targets_flat <= BIN_END) & (~is_pad)
...
# =========================================================
def compute_metrics(
    logits, targets, BIN_START, NUM_BINS, PAD_TOKEN, binner, EOS_TOKEN=1
):
    """
    logits:  (B, T, V)
    targets: (B, T)
    ...
    coord_mask = (
        (target_bin_rel >= 0) & (target_bin_rel < NUM_BINS) & (targets != PAD_TOKEN)
    )
...
def train(
    ...
    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        ...
        if use_strict_resume:
            ...
            if rank == 0:
                print(
                    f"[Rank {rank}] 🔁 Strict resume from {checkpoint_path}, epoch {start_epoch}"
                )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        pbar = tqdm(
            train_loader, disable=(rank != 0), desc=f"[Rank {rank}] Train {epoch}"
        )
        ...
        with torch.no_grad():
            pbar = tqdm(
                val_loader, disable=(rank != 0), desc=f"[Rank {rank}] Val {epoch}"
            )
            for batch in pbar:
                latents = batch["vae_mu"].to(device).squeeze(-1)
                decoder_input = batch["decoder_input_discrete"].to(device)
