...
# LOAD DATASET
# =========================
train_dataset, test_dataset = BarLinkageDataset.from_folder(data_dir)
...
Z = np.stack(
    [
        test_dataset[i]["vae_mu"].cpu().numpy().reshape(-1)
...
for i, batch in enumerate(tqdm(dataloader)):
    ...
    # ---------- QUERY ----------
    pred_vals_norm = predict_safe(model, latent, mech_idx)  # (T, 2) normalized
    ...
    for k, zidx in enumerate(idxs[0][1:]):
        # Access neighbor via test_dataset
        nb_sample = test_dataset[int(zidx)]
        nb_latent = nb_sample["vae_mu"].detach().clone().to(device)
        nb_mech_idx = int(nb_sample["encoded_labels"])
        nb_mech = index_to_label[str(nb_mech_idx)]

        nb_pred_norm = predict_safe(model, nb_latent, nb_mech_idx)

        nb_pred_padded = np.zeros((nb_pred_norm.shape[0], 3), dtype=np.float32)
        nb_pred_padded[:, :2] = nb_pred_norm
        nb_pred_denorm = nb_pred_padded[:, :2]
        nb_pts = nb_pred_denorm

        Pn = simulate_safe(nb_pts, nb_mech)
        if Pn is None:
            continue

        nci = coupler_index_for(nb_mech)
        if nci < 0:
            continue
...
# === FINAL METRIC SUMMARY (CONTINUOUS) ===
# QUERY CD | count=100 | mean=0.344957 | median=0.076042 | success(<0.01)=12
# QUERY DTW | count=100 | mean=0.630894 | median=0.096279 | success(<0.25)=61
# KNN MEAN CD | count=100 | mean=0.323454 | median=0.169259 | success(<0.01)=2
# KNN MEAN DTW | count=100 | mean=0.587500 | median=0.290027 | success(<0.25)=46

# === FINAL METRIC SUMMARY (DISCRETE) ===
